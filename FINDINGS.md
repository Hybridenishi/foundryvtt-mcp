...
**The sidecar image builds from files the repository does not ship.** `sidecar/Dockerfile:4` runs `npm ci --only=production`, which fails without a lockfile — but `sidecar/package-lock.json` has never been tracked in git, and `scripts/deploy-foundry.sh` neither preflights nor copies it or `sidecar/package.json`. The deploy script ships only the four sidecar `.js` files, the Dockerfile, and the module files. A rebuild therefore depends on a lockfile and manifest already present in the remote sidecar directory from earlier manual setup; on a fresh host it would fail. Deciding this properly means either committing the lockfile and adding both files to the deploy script's preflight and scp blocks, or switching the image to `npm install`, which gives up reproducible dependency pinning.
