...
**Two divergent write patterns.** The newer paths — `preview_hp_change`/`apply_hp_change` and `preview_item_activity_use`/`execute_item_activity_use` — use preview → scoped confirmation token → `dnd5e` API via the GM bridge. The five legacy writes (`update_actor`, `create_actor`, `delete_actor`, `next_turn`, `create_chat_message`) use raw `modifyDocument` with no preview, no confirmation, and no bridge. `next-turn` (`sidecar/app.js:610`) computes the turn and round increment in the sidecar itself rather than calling `Combat#nextTurn`, bypassing dnd5e hooks entirely.

**No reconnect.** The socket `disconnect` handler (`sidecar/index.js:94`) sets `connected = false` and stops. Recovery requires a container restart.

**No persistence, no audit trail.** Nothing anywhere writes to disk. All sidecar state — bridge clients, tokens, in-flight requests, every confirmation store — lives in in-memory Maps lost on restart. No mutation is recorded anywhere: not what changed, not who asked, not what it was before. The MCP server logs to stderr only; the sidecar uses bare `console.log`. The sidecar Dockerfile declares no volume.

**Full world dump per request.** `getWorld()` (`sidecar/index.js:101`) re-emits `"world"` on every request. There is no cache and no delta subscription, which is the main obstacle to the Phase 4 event tap.

**Single-consumer bridge dispatch.** `dispatchPreparedActorRequests()` (`sidecar/app.js:88`) serves the first client with a parked poll, one request per poll cycle. With an 8s request timeout against a 25s poll window, concurrent bridge operations serialize and can time out. Phases 2 and 4 add many more bridge operations, so this will bite.

**Broader route-level test coverage now exists, but the real Foundry socket flow is still unverified locally.** `node --test` runs 40 sidecar tests (up from roughly a dozen pure-function tests) plus 36 module tests. `sidecar/app.js` — the Express route table — is now a plain function of injected dependencies (`createApp(deps)`), which is what let `sidecar/app.test.js` exercise auth failure, write gating, route mapping, confirmation-token binding, and bridge timeouts entirely against fakes, with no live Foundry. Still untested: the real Socket.IO auth handshake in `sidecar/index.js`'s `connect()`, and the module's bridge poll loop against a live Foundry client. There is no CI. Tests still scan every sidecar source file to assert no shared credential strings are present (`sidecar/bridge-auth.test.js`).
