...
export function registerWriteTools(server: McpServer, client: FoundryClient, writeEnabled: boolean): void {
  ...
  server.registerTool(
    "preview_hp_change",
    {
      description: "Preview a direct D&D 5e HP damage or healing change through the active GM bridge. This does not modify Foundry and returns a short-lived confirmation token for apply_hp_change. When damageType is provided (damage mode only), dnd5e calculates resistance, vulnerability, and immunity. Standard dnd5e damage types: acid, bludgeoning, cold, fire, force, lightning, necrotic, piercing, poison, psychic, radiant, slashing, thunder.",
      inputSchema: {
        actorId: z.string().min(1),
        mode: z.enum(["damage", "healing"]),
        amount: z.number().int().min(1).max(100_000),
        damageType: z.string().min(1).optional().describe("dnd5e damage type (damage mode only; e.g., 'fire', 'piercing')"),
      },
  ...
  server.registerTool(
    "apply_hp_change",
    {
      ...
      inputSchema: {
        ...
        mode: z.enum(["damage", "healing"]),
        amount: z.number().int().min(1).max(100_000),
        damageType: z.string().min(1).optional().describe("dnd5e damage type (damage mode only; must match preview)"),
