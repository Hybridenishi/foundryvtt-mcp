...
The "execute one Item activity" proof of concept is now also validated live, closing out this phase's exit criteria. In the isolated test world, `preview_item_activity_use`/`execute_item_activity_use` ran an unambiguous, self-targeted dnd5e utility activity (Cunning Action's Midi Use, on an actor named Exodus) end to end through the active GM bridge: dnd5e's `Activity#use()` executed for real, posted a chat card, and reported no resource consumption or effects, matching the activity's own configuration. The first attempt failed with Foundry's internal `PlaceableObject` construction error; the actor had no token on the active scene. dnd5e's `Activity#use()` requires a placed token to resolve self-targeting even when no external target or template is configured. The guard now checks `actor.getActiveTokens()` and rejects with a clear error at preview time instead of letting execution fail with that internal error message.
